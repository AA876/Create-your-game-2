...
var boy, boypose1, boypose2, boypose3, boypose4, boypose5, boyrollingimg, boyjumpingimg
var shark, sharkimg1
...
junglebackground = createSprite(width/2,height,width,20)
...
if (gameState===JUNGLE){
    ...
    //tiger = createSprite(100,520,50,50)
    //tiger.addAnimation("tiger", tigerpose);
    //tiger.setCollider('circle',0,0,120)
    //tiger.scale = 0.35
  
    //if(tiger.isTouching(boy)){
        //
    //}
  ...
  if (gameState===UNDERWATER){
    background(underwaterbackgroundimg)
    fill("black")
    textSize(18)
    text("Score: "+ score, 1100,50);
    score = score + Math.round(getFrameRate()/60);
    ...
    //shark = createSprite(100,520,50,50)
    //shark.addAnimation("tiger", tigerpose);
    //shark.setCollider('circle',0,0,120)
    //shark.scale = 0.35
  ...
  function spawnHangingObstacles(){
    if(frameCount % 200 === 0) {
      var hangingObstacle = createSprite(600,height-270,20,30);
  ...
  //need to add tiger
  //need to add shark
  //need to add alien, preload the image, crop and add it in the space gamestate